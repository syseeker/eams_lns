...
#    cfg_data   -->  number of test cases in line[0], follow by a list of test cases to run 
    ...
    for i in range(1, runCount+1):        
        ...
        if RUNTYPE == 0:
            ...
            milp_solver.initModel()
            status = milp_solver.getInitialSchedule(0) 
            if status == STATUS.index('INFEASIBLE'):
                print "Failure to find an initial schedule."  
            else:
